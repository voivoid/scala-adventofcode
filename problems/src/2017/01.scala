...
object problem01 extends baseProblem {
  ...
  override def solve1(input: Input): Int = {
    ...
    val pairs = cycledStr.take(inputLen + 1).sliding(2)
    sumPairs(pairs.map { case Seq(a, b) => (a, b) })
  }
  ...
  override def solve2(input: Input): Int = {
    ...
    val pairs = iter1.take(inputLen).zip(iter2.drop(inputLen / 2))
    sumPairs(pairs)
  }

  private def sumPairs(pairs: Iterator[(Char, Char)]): Int = {
    pairs.map { case (a, b) => if (a == b) a.asDigit else 0 }.sum
  }

  private def readInput(input: Input): (Int, Iterator[Char]) = {
    val inputStr = input.mkString
    val cycledStr = inputStr.iterator.concat(inputStr.iterator)
