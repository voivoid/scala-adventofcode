object ProblemsApp {
  ...
  def main(args: Array[String]): Unit = {
    args.length match {
      ...
      case 2 => {
        val result = scala.util.Using(io.Source.fromFile(args(1))) {
          source => solveProblem(args(0), source)
        }
  ...
  private def solveProblem(problemId: String, input: scala.io.Source): Unit = {
    ...
    val problem = try {
      ...
      runtimeMirror.reflectModule(module).instance.asInstanceOf[adventOfCode.problems.baseProblem]
    }
    catch {
      case e: Throwable => sys.error(s"Failed to load the problem ${problemStr}: ${e.getMessage}")
    }
